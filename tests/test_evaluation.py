import pytest
from figchain.evaluation import Evaluator
...
def test_evaluate_condition_contains(evaluator):
    cond = Condition(
        variable="email", operator=Operator.CONTAINS, values=["@example.com"]
    )
...
def test_evaluate_condition_split(evaluator):
    ...
    bad_cond = Condition(
        variable="user_id", operator=Operator.SPLIT, values=["not-a-number"]
    )
...
def test_evaluate_family_rules(evaluator):
    ...
    rule = Rule(
        conditions=[
            Condition(variable="user_id", operator=Operator.EQUALS, values=["ben"])
        ],
